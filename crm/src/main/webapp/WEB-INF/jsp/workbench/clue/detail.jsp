...
<head>
    ...
    <script type="text/javascript">
        jQuery(function ($) {
            ...
            function load() {
                $.ajax({
                    url: "/activities/relationClue.json?id=${param.id}",
            ...
            $("#glBtn").click(function () {
                $.ajax({
                    url: "/activities/getList.json",
                    success: function (result) {
                        var arr = [];
                        $(result).each(function () {
                            arr.push(`
                                    <tr>
                                     <td><input name="act" value="` + this.id + `" type="checkbox"/></td>
                                     <td>` + this.name + `</td>
                                     <td>` + this.startDate + `</td>
                                     <td>` + this.endDate + `</td>
                                     <td>` + this.owner + `</td>
                        ...
                        $("#tb2").html(arr.join(""));
                        // 初始化选中状态
                        $(":checkbox[name=act]").prop("checked", function () {
                            return $("tr[actId=" + this.value + "]").length == 1;
                        });
            ...
            $("#likeSearch").on("input", function () {
                var v = this.value;
                $("#tb2 tr").hide().filter(function () {
                    return $(this).find("td:eq(1):contains(" + v + ")").size() > 0;
                }).show();
            });

            $("#guanlianBtn").click(function () {
                //    获取选中的复选框
                var activityId = $(":checkbox[name=act]:checked").map(function () {
                    return this.value;
                }).get().join();
                $.ajax({
            ...
            $("#checkAll").click(function () {
                $(":checkbox[name=act]").prop("checked", this.checked);
            });

            $("#activityTable").on("click", ":checkbox[name=act]", function () {
                $("#checkAll").prop("checked", $(":checkbox[name=act]").length == $(":checkbox[name=act]:checked").length);
            });
...
<div class="modal fade" id="bundModal" role="dialog">
    <div class="modal-dialog" role="document" style="width: 80%;">
        <div class="modal-content">
                ...
                <div class="btn-group" style="position: relative; top: 18%; left: 8px;">
                    <form class="form-inline" role="form">
                        <div class="form-group has-feedback">
                            <input id="likeSearch" autocomplete="off" type="text" class="form-control"
                                   style="width: 300px;"
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                <button id="guanlianBtn" type="button" class="btn btn-primary">关联</button>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10" style="width: 300px;">
                            <select class="form-control" id="edit-call">
                                <option></option>
                                <c:forEach items="${zd.source}" var="z">
                                    <option>${z}</option>
                                </c:forEach>
                            </select>
                        </div>
...
<div style="position: relative; left: 40px; top: -30px;">
    ...
    <div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
        ...
        <button type="button" class="btn btn-default" onclick="window.location.href='convert.html?clueId=${param.id}';"><span
...
<div>
    <div style="position: relative; top: 60px; left: 40px;">
        ...
        <div>
            <a href="###" id="glBtn" data-toggle="modal" data-target="#bundModal"
